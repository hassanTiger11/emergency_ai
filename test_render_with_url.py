#!/usr/bin/env python3
"""
Test Render Database with Custom URL
"""
import requests
import json
...
def test_render_database():
    ...
    try:
        ...
        print("   This might mean your app is not deployed or not running")
        return
    
    # Test 2: Database Connection
    print("\n2. Testing Database Connection...")
    try:
        response = requests.get(f"{render_url}/api/auth/me", timeout=15)
    ...
    try:
        response = requests.post(f"{render_url}/api/auth/signup", json=register_data, timeout=20)
        ...
        if response.status_code == 201:
            ...
            response = requests.post(f"{render_url}/api/auth/login", json=login_data, timeout=15)
            if response.status_code == 200:
                auth_data = response.json()
                token = auth_data['access_token']
    ...
    try:
        response = requests.get(f"{render_url}/api/user/profile", headers=headers, timeout=15)
        if response.status_code == 200:
            user_data = response.json()
    ...
    try:
        response = requests.post(f"{render_url}/api/save-analysis", data=analysis_data, headers=headers, timeout=20)
        if response.status_code == 200:
    ...
    try:
        response = requests.get(f"{render_url}/api/user/conversations", headers=headers, timeout=15)
        if response.status_code == 200:
